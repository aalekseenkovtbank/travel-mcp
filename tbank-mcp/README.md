...
### Машиночитаемые ответы для Travel Nova

Read-only инструменты профиля и путешествий принимают необязательный
...
формат существующих человекочитаемых сценариев не меняется.

Все hotel-инструменты, возвращающие отель, добавляют статический `details` с
адресом, описанием, временем заезда/выезда, координатами, удобствами, точным
T-Bank URL и максимум тремя реальными HTTPS-фотографиями. У `hotel_rates` и
`hotel_reviews` блок называется `hotelDetails`. Списки загружают детали пакетно;
при сбое основная выдача сохраняется с warning и `meta.detailsComplete=false`.

Сценарные `compare_flight_prices`, `compare_train_prices`,
`compare_hotel_prices` и `compare_flight_hotel_prices` сразу возвращают
типизированный `structuredContent` с JSON fallback. Они выполняют fan-out по
датам, фильтрацию, сортировку и расчёт RUB/%-дельт внутри Python. Поля называются
`lowest observed`: при неполной выдаче сервер не заявляет глобальный минимум.
...
| `tbank-hotel-search` | Public Hotels search — HTML shortlist of five by default, chat on request |
