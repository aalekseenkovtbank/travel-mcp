...
def test_train_search_prices_and_seats_are_not_all_empty():
    ...
    server._search_trains = lambda **kwargs: {
        "data": {"origin": kwargs["origin"], "destination": kwargs["destination"],
                 "date": kwargs["departure_date"], "adults": kwargs["adults"],
                 "children": kwargs["children"], "ways": ways},
        "warnings": [], "source": "T-Bank Railways", "complete": True,
    }
    try:
        out = asyncio.run(server.train_search(
...
def test_hotel_search_and_details_render_the_public_shapes():
    autocomplete = {
        "locations": [{"id": 17039, "name": "Москва",
                       "signature": "Город, Москва, Россия",
                       "type": {"name": "Город", "code": "city"}}],
        "hotels": [{"id": "1471735", "name": "Cosmos Smart",
                    "signature": "Отель • Москва",
                    "type": {"name": "Отель", "code": "hotel"}}],
    }
    out = run(server.hotel_autocomplete,
              Stub(hotel_autocomplete=autocomplete), "Москва")
    check("id=17039" in out and "id=1471735" in out,
          f"autocomplete must print ids for both result kinds: {out!r}")

    search = {
        "filteredHotelsCount": 1,
        "isLoadingCompleted": False,
        "hotels": [{
            "hotelId": "1471735", "hotelName": "Cosmos Smart",
            "starRating": 4,
            "hotelLocation": {"address": "ул. Щипок, 22"},
            "review": {"rating": 9.1},
            "rateForHotelsFeed": {
                "shownPrice": {"amount": "12345.67", "currency": "RUB"},
                "mealName": "Завтрак включён", "availableRoomsCount": 3,
            },
        }],
    }
    out = run(server.hotel_search, Stub(hotel_search=search), 17039,
              "2026-08-19", "2026-08-20", adults=2, children_ages="5,12")
    check("12346 RUB" in out and "hotel_id=1471735" in out,
          f"hotel price and reusable id must render: {out!r}")
    check("первый" in out and "не гарантия полного" in out,
          f"an incomplete asynchronous result must say so: {out!r}")
    check("не выполняются" in out,
          f"read-only search must not imply it can book or charge: {out!r}")

    details = {
        "hotelId": "1471735", "hotelName": "Cosmos Smart", "starRating": 4,
        "hotelLocation": {"address": "ул. Щипок, 22"},
        "checkInTime": "14:00", "checkOutTime": "12:00",
        "description": "Удобный городской отель.",
        "facilitiesGroups": [{"groupName": "Интернет",
                              "facilities": [{"name": "Бесплатный Wi-Fi"}]}],
    }
    out = run(server.hotel_details, Stub(hotel_details=details), "1471735")
    check("ул. Щипок" in out and "Интернет: Бесплатный Wi-Fi" in out,
          f"hotel address and facilities must render: {out!r}")
    check("Бронирование и оплата" in out,
          f"details must keep the read-only boundary visible: {out!r}")

    bad = run(server.hotel_search, Stub(hotel_search={}), 17039,
...
def main():
    ...
    test_hotel_search_and_details_render_the_public_shapes()
