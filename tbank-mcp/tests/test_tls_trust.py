...
def test_a_hostile_certificate_is_not_learned():
    ...
    try:
        ...
        try:
            ...
            os.makedirs(roots)
            shutil.copy(os.path.join(tls.ROOTS_DIR, "russian-trusted-root-ca.crt"), roots)
...
def test_a_tampered_shipped_root_is_refused():
    ...
    try:
        roots = os.path.join(tmp, "roots")
        os.makedirs(roots)
        name = "russian-trusted-root-ca.crt"
...
def test_macos_keychain_requires_explicit_tls_trust():
    """Being present in a Keychain is not sufficient to become a CA bundle anchor."""
    tmp = tempfile.mkdtemp()
    try:
        _, trusted_crt = make_self_signed(tmp, cn="Trusted Corporate Root")
        _, merely_present_crt = make_self_signed(tmp, cn="Merely Present Certificate")
        trusted = open(trusted_crt, encoding="utf-8").read()
        merely_present = open(merely_present_crt, encoding="utf-8").read()

        def sha1(pem):
            return hashlib.sha1(
                ssl.PEM_cert_to_DER_cert(pem), usedforsecurity=False
            ).hexdigest().upper()

        trust_settings = plistlib.dumps({
            "trustVersion": 1,
            "trustList": {
                # No trustSettings means unconditional trust in Apple's export.
                sha1(trusted): {"issuerName": b"trusted"},
            },
        })
        selected = tls.select_macos_trusted_certificates(
            trusted + "\n" + merely_present, [trust_settings]
        )
        check(len(selected) == 1 and trusted.strip() in selected[0],
              "a trusted root must be selected and a merely present cert excluded")

        denied_settings = plistlib.dumps({
            "trustVersion": 1,
            "trustList": {
                sha1(trusted): {
        ...
        check(tls.select_macos_trusted_certificates(trusted, [denied_settings]) == [],
              "an explicitly denied Keychain certificate became a trust anchor")
    finally:
        shutil.rmtree(tmp, ignore_errors=True)
    print("  macOS Keychain: explicit trust selected; presence-only and deny excluded")
...
def test_bundle_contains_only_trusted_material():
    ...
    try:
        ...
        keychain_n = len(tls.load_macos_keychain_roots())
        check(n == sys_n + keychain_n + len(tls.load_roots()),
              f"bundle has {n} certs, expected {sys_n} system + "
              f"{keychain_n} Keychain + {len(tls.load_roots())} roots — "
              f"something extra got in")
        ...
        check(tls.fingerprint(open(
            os.path.join(tls.ROOTS_DIR, "russian-trusted-root-ca.crt"),
            encoding="utf-8").read())
            == tls.PINNED_ROOTS["russian-trusted-root-ca.crt"],
            "the shipped root no longer matches its recorded pin")
    finally:
        shutil.rmtree(tmp, ignore_errors=True)
    print("  bundle: system store + trusted Keychain CAs + pinned roots; no peer leaves")
...
def main():
    ...
    test_macos_keychain_requires_explicit_tls_trust()
