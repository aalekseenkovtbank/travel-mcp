...
Передай фактические события в `request.events`. Если блок остался пустым,
`get_trip_report` сам выполнит ограниченный поиск концертов и спектаклей,
подтвердит расписание и заполнит Афишу без `search_app`.
...
`get_trip_report` не перепроверяет цены и тарифы отелей: обнови их до вызова.
Если `request.events` пуст, report автоматически загрузит подтверждённые
концерты и спектакли Афиши. Если `request.venues` пуст, report вызовет публичный
`restaurant_search()` вокруг выбранного отеля и включит найденные рестораны в
HTML и Markdown; уже переданные заведения без фото сопоставит с публичными
карточками Яндекс.Карт. Ошибку источника перенесёт в warnings. В HTML фотографии
отелей, событий и ресторанов встраиваются с доверенных CDN; исходные HTTPS URL
сохраняются в `documentJson`.
...
контракте нет.
В HTML report сам встраивает доступные фотографии отелей, событий Афиши и
