...
So the bundle is: **system CAs + explicitly trusted macOS Keychain certificates +
the pinned Russian root**, and nothing else, which passes every host in
`BANK_HOSTS`. The Keychain matters on corporate Macs: curl uses it, while Python's
OpenSSL does not. Certificates merely present in a Keychain are not enough — their
fingerprints must also occur in the user/admin trust settings. The host list is
derived from the endpoint table rather than typed out — see bank_hosts() for what
the typed-out version cost.
...
def _normalized_sha1(value) -> str:
    return re.sub(r"[^0-9a-f]", "", str(value).lower())


def _trust_entry_allows_tls(entry) -> bool:
    """Whether one macOS trust-settings entry may act as a TLS trust anchor.

    An absent `trustSettings` means unconditional trust. Explicit settings are
    accepted only when they trust a root/as-root globally or for an SSL policy.
    An explicit deny wins. Unknown policy blobs are skipped rather than broadened
    into trust for every HTTPS connection.
    """
    ...
    for setting in settings:
        if not isinstance(setting, dict):
            continue
        policy_name = setting.get("kSecTrustSettingsPolicyName")
        has_unknown_policy = (
            "kSecTrustSettingsPolicy" in setting and policy_name is None
        )
        applies_to_tls = not has_unknown_policy and (
            policy_name is None or "ssl" in str(policy_name).lower()
        )
        if not applies_to_tls:
            continue
        result = setting.get("kSecTrustSettingsResult")
        if result == 3:  # kSecTrustSettingsResultDeny
            return False
        if result in (None, 1, 2):  # default, TrustRoot, TrustAsRoot
            allowed = True
    return allowed


def select_macos_trusted_certificates(
    pem_text: str, trust_settings_payloads: list[bytes]
) -> list[str]:
    """Select Keychain PEMs explicitly present in user/admin trust settings.

    SHA-1 is used only as Apple's trust-list identifier, never cryptographically.
    """
    trusted: set[str] = set()
    for payload in trust_settings_payloads:
        try:
            plist = plistlib.loads(payload)
        except (ValueError, TypeError, OverflowError, plistlib.InvalidFileException):
            continue
        trust_list = plist.get("trustList", {}) if isinstance(plist, dict) else {}
        if not isinstance(trust_list, dict):
            continue
        for fingerprint_value, entry in trust_list.items():
            fingerprint_value = _normalized_sha1(fingerprint_value)
            if len(fingerprint_value) == 40 and _trust_entry_allows_tls(entry):
                trusted.add(fingerprint_value)

    selected: list[str] = []
    seen: set[str] = set()
    for block in _FULL_PEM_RE.findall(pem_text):
        try:
            der = ssl.PEM_cert_to_DER_cert(block)
        except ValueError:
            continue
        fingerprint_value = hashlib.sha1(  # noqa: S324 - macOS lookup key only
            der, usedforsecurity=False
        ).hexdigest()
        if fingerprint_value in trusted and fingerprint_value not in seen:
            selected.append(block + "\n")
            seen.add(fingerprint_value)
    return selected


def load_macos_keychain_roots() -> list[str]:
    """Export explicitly trusted user/admin certificates on macOS.

    `/usr/bin/security` reads only local trust configuration. It never contacts the
    failing host, so a peer cannot install itself by causing an SSL error.
    """
    ...
    payloads: list[bytes] = []
    try:
        with tempfile.TemporaryDirectory(prefix="tbank-keychain-") as directory:
            for domain_args, filename in (
                (("-d",), "admin.plist"),
                ((), "user.plist"),
            ):
                path = os.path.join(directory, filename)
                result = subprocess.run(
                    [security, "trust-settings-export", *domain_args, path],
        ...
        _log(f"macOS Keychain export failed: {e}")
        return []
    if result.returncode != 0:
        _log("macOS Keychain certificates could not be exported; using file-based CAs")
        return []

    certificates = select_macos_trusted_certificates(
        result.stdout.decode("utf-8", errors="replace"), payloads
    )
    if certificates:
        _log(f"trusting {len(certificates)} certificate(s) from macOS trust settings")
    return certificates
...
def rebuild_bundle(hosts=None, out: str | None = None) -> str:
    """Write the CA bundle: system store + macOS trust settings + pinned roots.

    `hosts` is accepted and ignored — the old signature took the hosts to go and
    fetch certificates from, which is the behaviour that was removed. The macOS
    `security` subprocess reads local trust settings only; it never talks to a peer.
    """
    ...
    parts.extend(load_macos_keychain_roots())
    roots = load_roots()
    parts.extend(roots)
    if not roots:
