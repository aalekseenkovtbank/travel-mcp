...
FORMER_TRAVEL_TOOL_NAMES = frozenset({
    ...
    # Events and places.
    "search_app", "cinema_search", "cinema_schedule", "cinema_seats",
...
TOOL_KINDS: dict[str, tuple[str, str]] = {
    ...
    "audience_profile": ("Безопасная возрастная группа клиента", READ),
    # cards and documents
    ...
    "hotel_search": ("Отели с деталями, фото, тарифами и отзывами", READ),
...
def _restaurant_enriched_report(request: TravelPageDocument) -> TravelPageDocument:
    ...
    try:
        ...
        message = getattr(exc, "message", str(exc))
        warning = f"Рестораны Яндекс.Карт не загружены: {_cut(message, 220)}"
        warnings = list(payload.get("warnings") or [])
        if warning not in warnings:
            payload["warnings"] = [*warnings[:23], warning]
        return type(request).model_validate(payload)

    restaurants = list((result.get("data") or {}).get("restaurants") or [])
    payload["venues"] = restaurants
    map_points = list(payload.get("mapPoints") or [])
    mapped_ids = {str(point.get("refId") or "") for point in map_points}
    for restaurant in restaurants:
        if len(map_points) >= 32:
            break
        ref_id = str(restaurant.get("id") or "")
        kind = str(restaurant.get("kind") or "restaurant")
        if ref_id and ref_id not in mapped_ids:
            map_points.append({"refId": ref_id, "kind": kind})
...
def get_trip_report(
    ...
    собираются из транспорта, выбранного отеля, событий и заведений: от
    прибытия и заселения через прогулки, мероприятия и еду до выселения и
...
def _safe_age_band(birth_date) -> tuple[str, bool | None]:
    """Collapse a birth date to the minimum useful audience signal.

    The exact date and sex stay inside the MCP process and never enter the
    Travel Nova profile or its database.
    """
    ...
    if not match:
        return "unknown", None
    try:
        born = datetime.strptime(match.group(0), "%Y-%m-%d").date()
    except ValueError:
        return "unknown", None
...
@mcp.tool()
def audience_profile(response_format: str = "text") -> str:
    """Безопасный возрастной признак для ограничений афиши.

    Возвращает только возрастной диапазон и допустимость 18+. Точная дата
    рождения, имя, контакты, документы и пол не возвращаются.
    """
    try:
        fmt = _response_format(response_format)
        s = _require(); s.ensure_client_session()
        brief = s.identity_brief() or {}
        birth = brief.get("birthDate") or {}
        birth_value = birth.get("value") if isinstance(birth, dict) else birth
        age_band, adult_allowed = _safe_age_band(birth_value)
        data = {"ageBand": age_band, "adultContentAllowed": adult_allowed}
        if fmt == "json":
            return _json_envelope(data, source="T-Bank Audience Profile")
        adult = "да" if adult_allowed is True else "нет" if adult_allowed is False else "неизвестно"
        return f"Возрастная группа: {age_band}; можно 18+: {adult}."
    except Exception as e:
        return _formatted_error(e, response_format, source="T-Bank Audience Profile")

@mcp.tool()
def list_accounts(response_format: str = "text") -> str:
    ...
    try:
        ...
        if fmt == "json":
            accounts = []
            for account in accs or []:
                currency = account.get("currency") or {}
                money = account.get("moneyAmount") or {}
                accounts.append({
                    "id": str(account.get("id") or ""),
                    "accountType": str(account.get("accountType") or ""),
                    "name": _flat(account.get("name") or ""),
                    "balance": money.get("value"),
                    "currency": (str(currency.get("name") or "") if isinstance(currency, dict)
                                 else str(currency or "")),
                    "cards": [{
                        "id": str(card.get("id") or ""),
                        "ucid": str(card.get("ucid") or ""),
                        "name": _flat(card.get("name") or ""),
                        "status": str(card.get("status") or ""),
                    } for card in (account.get("cards") or [])
                       if isinstance(card, dict)],
                })
            return _json_envelope({"accounts": accounts}, source="T-Bank Accounts")
        if not accs:
        ...
        return _formatted_error(e, response_format, source="T-Bank Accounts")

@mcp.tool()
def list_operations(account_id: str, days: int = 30, limit: int = 50,
                    desc_len: int = 40, response_format: str = "text") -> str:
    ...
    try:
        ...
        if fmt == "json":
            ...
            for operation in shown:
                ...
                category_name = (category.get("name") if isinstance(category, dict)
                                 else str(category or ""))
                rows.append({
                    "id": str(operation.get("id") or ""),
                    "occurredAt": when(operation),
                    "type": str(operation.get("type") or ""),
                    "amount": amount.get("value"),
                    "currency": (currency.get("name") if isinstance(currency, dict)
                                 else str(currency or "")),
                    "description": _cut(operation.get("description") or "", desc_len),
                    **({"category": _flat(category_name)} if category_name else {}),
                })
            warnings = ([] if len(shown) == len(ops) else
                        [f"Показано {len(shown)} из {len(ops)} операций."])
            return _json_envelope({
                "accountId": str(account_id),
                "days": days,
                "total": len(ops),
                "operations": rows,
            }, source="T-Bank Operations", warnings=warnings,
               meta={"complete": len(shown) == len(ops)})
        if not ops:
        ...
        return _formatted_error(e, response_format, source="T-Bank Operations")

@mcp.tool()
def spending_categories(account_id: str, days: int = 30,
    ...
    try:
        ...
        if fmt == "json":
            return _json_envelope({
                "accountId": str(account_id),
                "days": days,
                "totalSpent": rep.get("total_spent") or 0,
                "totalEarned": rep.get("total_earned") or 0,
                "currency": str(rep.get("currency") or "RUB"),
                "categories": [{
                    "name": _flat(category.get("category") or ""),
                    "amount": category.get("amount") or 0,
                    "sharePct": category.get("share_pct") or 0,
                } for category in cats if isinstance(category, dict)],
            }, source="T-Bank Spending Analytics")
        ...
        return _formatted_error(e, response_format, source="T-Bank Spending Analytics")
...
@mcp.tool()
def orders(kind: str = "", limit: int = 10, response_format: str = "text") -> str:
    ...
    try:
        ...
        if fmt == "json":
            shown = picked[:limit] if limit > 0 else picked
            rows = []
            for order in shown:
                fields = order.get("fields") or {}
                title = (fields.get("eventName") or fields.get("hotelName")
                         or fields.get("objectName") or fields.get("applicationName")
                         or fields.get("partnerName") or "")
                rows.append({
                    "orderId": str(order.get("orderId") or ""),
                    "objectType": str(order.get("objectType") or ""),
                    "status": str(order.get("status") or ""),
                    "amount": order.get("amount"),
                    "createdAt": str(order.get("created") or ""),
                    "title": _flat(title),
                    "eventName": _flat(fields.get("eventName") or ""),
                    "hotelName": _flat(fields.get("hotelName") or ""),
                    "destination": _flat(fields.get("destination")
                                         or fields.get("arrivalCity") or ""),
                })
            warnings = ([] if len(shown) == len(picked) else
                        [f"Показано {len(shown)} из {len(picked)} заказов."])
            return _json_envelope({
                "kind": kind,
                "total": len(picked),
                "orders": rows,
            }, source="T-Bank Orders", warnings=warnings,
               meta={"complete": len(shown) == len(picked)})
        if not picked:
        ...
        return _formatted_error(e, response_format, source="T-Bank Orders")

@mcp.tool()
def order_details(order_id: str, response_format: str = "text") -> str:
    ...
    try:
        ...
        elements = [element for element in (cart.get("cartElement") or [])
                    if isinstance(element, dict)]
        if fmt == "json":
            geo = obj.get("geo") or {}
            return _json_envelope({
                "orderId": str(info.get("orderId") or order_id),
                "status": str(info.get("status") or ""),
                "createdAt": str(info.get("created") or ""),
                "eventName": _flat(ev.get("eventName") or f.get("eventName") or ""),
                "genres": [_flat(value) for value in (ev.get("genres") or []) if value],
                "venue": _flat(obj.get("objectName") or ""),
                "address": _flat(geo.get("address") or ""),
                "startDateTime": str(info.get("reserveDate") or ""),
                "hallName": _flat(f.get("hallName") or ""),
                "seatCount": len(elements),
                "ticketPricesRub": [element.get("price") for element in elements
                                    if element.get("price") is not None],
                "totalAmountRub": cart.get("amount") or info.get("amount") or 0,
            }, source="T-Bank Event Order")
        ...
        return _formatted_error(e, response_format, source="T-Bank Event Order")
...
@mcp.tool()
def travel_order_details(order_id: str, response_format: str = "text") -> str:
    ...
    try:
        ...
        if kind in _TRAVEL_BLOCKED:
            if fmt == "json":
                return _json_envelope({
                    "orderId": str(order_id), "kind": str(kind),
                    "status": str(order.get("status") or ""),
                    "amountRub": order.get("amount") or 0,
                    "createdAt": str(order.get("created") or ""),
                    "title": _flat(f.get("eventName") or f.get("hotelName") or ""),
                    "destination": _flat(f.get("destination") or f.get("arrivalCity") or ""),
                    "detailsAvailable": False,
                }, source="T-Bank Travel Orders",
                   warnings=[_TRAVEL_BLOCKED[kind]])
        ...
        if not b:
            if fmt == "json":
                raise ValueError(f"Бронь {order_id} не найдена")
            return head + "\nБронь не найдена на hotels.t-bank-app.ru."
        h = b.get("hotelData") or {}
        area = h.get("areaLocation") or {}
        rate = b.get("rateData") or {}
        rooms = [room for room in (rate.get("rooms") or []) if isinstance(room, dict)]
        guests = [guest for room in rooms for guest in (room.get("guests") or [])
                  if isinstance(guest, dict)]
        meals = []
        for source in (rate, *(room for room in rooms)):
            for key in ("mealName", "mealType", "meal"):
                value = source.get(key) if isinstance(source, dict) else None
                if value and _flat(value) not in meals:
                    meals.append(_flat(value))
        if fmt == "json":
            return _json_envelope({
                ...
                "amountRub": order.get("amount") or 0,
                "createdAt": str(order.get("created") or ""),
                "title": _flat(h.get("hotelName") or f.get("hotelName") or ""),
                "destination": _flat(area.get("destinationName") or f.get("destination") or ""),
                "detailsAvailable": True,
                "checkInDate": str(b.get("checkInDate") or ""),
                "checkOutDate": str(b.get("checkOutDate") or ""),
                "hotelName": _flat(h.get("hotelName") or ""),
                "hotelStars": int(h.get("starRating") or 0),
                "mealTypes": meals,
                "roomCount": len(rooms),
        ...
        return _formatted_error(e, response_format, source="T-Bank Travel Orders")
...
def concert_schedule(event_id: str, kind: str = "concert",
    ...
    try:
                ...
                for event in venue.get("events") or []:
                    for slot in event.get("slots") or []:
                        ...
                        showings.append({
                            ...
                            "objectId": str(info.get("objectId") or ""),
                            "venue": _flat(info.get("objectName") or ""),
                            "address": _flat(geo.get("address") or ""),
                            "latitude": latitude,
                            "longitude": longitude,
                            "startDateTime": str(slot.get("startDateTime") or ""),
                            "slotId": str(slot.get("slotId") or ""),
                            "priceFix": (_hotel_amount(prices.get("fix"))
                                         if prices.get("fix") is not None else None),
                            "priceMin": (_hotel_amount(prices.get("min"))
                                         if prices.get("min") is not None else None),
                            "priceMax": (_hotel_amount(prices.get("max"))
                                         if prices.get("max") is not None else None),
                        })
            warnings = ([] if len(shown_venues) == len(venues) else
                        [f"Показано {len(shown_venues)} из {len(venues)} площадок."])
            return _json_envelope({
                "eventId": str(event_id),
                "kind": kind,
                "showings": showings,
            }, source="T-Bank Afisha", warnings=warnings,
               meta={"complete": len(shown_venues) == len(venues)})
        if not venues:
        ...
        return _formatted_error(e, response_format, source="T-Bank Afisha")
...
_HOTEL_REVIEW_THEMES: tuple[tuple[str, tuple[str, ...]], ...] = (
    ("удобное расположение", ("располож", "локац", "местополож", "центр", "метро")),
    ("чистота", ("чист", "уборк")),
    ("работа персонала", ("персонал", "администратор", "сотрудник", "сервис")),
    ("комфорт и оснащение номера", ("номер", "кровать", "матрас", "простор")),
    ("завтрак и питание", ("завтрак", "питани", "еда", "кофе")),
    ("тишина", ("тихо", "тишин")),
    ("шумоизоляция", ("шум", "слыш", "стен")),
    ("интернет", ("wifi", "wi-fi", "вайф", "интернет")),
    ("климат в номере", ("кондиционер", "жарк", "холод")),
    ("ванная и сантехника", ("душ", "ванн", "сантех")),
)

_HOTEL_TRAVELER_LABELS = {
    "family": "семьям",
    "couple": "парам",
    "company": "компаниям друзей",
    "solo_travel": "самостоятельным путешественникам",
    "business": "деловым путешественникам",
}


def _hotel_repeated_themes(reviews: list[dict], field: str) -> list[str]:
    counts: Counter[str] = Counter()
    for review in reviews:
        text = str(review.get(field) or "").strip().lower()
        if not text or text == "-":
            continue
        for label, stems in _HOTEL_REVIEW_THEMES:
...
def _hotel_review_digest(reviews: list[dict]) -> dict:
    ...
    traveler_counts = Counter(
        str((review.get("bookingInfo") or {}).get("travelerType") or "")
        for review in reviews
    )
    suitable_for = [
        label for value, label in _HOTEL_TRAVELER_LABELS.items()
        if traveler_counts[value] >= 2
    ][:3]
    dates = sorted(
        str((review.get("bookingInfo") or {}).get("createdDate") or "")[:10]
        for review in reviews
...
def _hotel_rate_summary(data: dict, nights: int) -> dict:
    rates = [row for row in (data.get("rates") or []) if isinstance(row, dict)]
    other_rates = [row for row in (data.get("otherRates") or []) if isinstance(row, dict)]
    rate = (rates or other_rates or [{}])[0]
    rooms = {
        str(room.get("roomId")): room for room in (data.get("rooms") or [])
        if isinstance(room, dict) and room.get("roomId") is not None
    }
    room = rooms.get(str(rate.get("roomId") or ""), {})
    shown = rate.get("shownPrice") or {}
    amount = _hotel_amount(shown)
    cancellation = rate.get("cancellationPolicyRules") or {}
    return {
        "available": bool(rate),
        "totalPrice": amount or None,
        "nightlyPrice": (round(amount / nights, 2) if amount and nights else None),
        "currency": str(shown.get("currency") or "RUB") if isinstance(shown, dict) else "RUB",
        "room": _flat(room.get("roomName") or ""),
        "bed": _flat(room.get("bedName") or ""),
        "meal": _flat(rate.get("mealName") or rate.get("mealType") or ""),
        "payment": str(rate.get("paymentPlace") or ""),
        "freeCancellationUntil": (
            str(cancellation.get("freeCancellationUntil") or "")
            if isinstance(cancellation, dict) else ""
        ),
        "nonRefundable": rate.get("isNonRefundable") is True,
        "availableRoomsCount": rate.get("availableRoomsCount"),
    }
...
def _hotel_markdown_cell(value: Any) -> str:
...
def _hotel_photo_alt(value: Any) -> str:
    return (
        str(value or "отель")
        .replace("\\", "")
        .replace("[", "")
...
def _hotel_enriched_text(items: list[dict]) -> str:
    lines = [
        ...
        "## Краткое сравнение",
        "",
        "| Отель | Полная цена | За ночь | Рейтинг | Отзывов | Номер | Питание |",
        "|---|---:|---:|---:|---:|---|---|",
    ]
    for item in items:
        ...
        lines.append(
            "| " + " | ".join([
                _hotel_markdown_cell(name),
                _hotel_markdown_cell(
                    f"{price:.0f} {currency}"
                    if isinstance(price, (int, float)) else "нет данных"),
                _hotel_markdown_cell(
                    f"{nightly:.0f} {currency}"
                    if isinstance(nightly, (int, float)) else "нет данных"),
                _hotel_markdown_cell(
                    item.get("rating")
                    if item.get("rating") is not None else "нет данных"),
                _hotel_markdown_cell(
                    review_count if review_count is not None else "нет данных"),
                _hotel_markdown_cell(rate.get("room") or "нет данных"),
                _hotel_markdown_cell(rate.get("meal") or "нет данных"),
            ]) + " |"
        )
    for position, item in enumerate(items, start=1):
        ...
        if rate.get("available"):
            ...
            price_text = (
                f"{price:.0f} {currency}"
                if isinstance(price, (int, float)) else "источник не вернул")
            nightly_text = (
                f"{nightly:.0f} {currency}"
                if isinstance(nightly, (int, float)) else "источник не вернул")
        else:
            price_text = "источник не вернул подтверждённый тариф"
            nightly_text = "источник не вернул подтверждённый тариф"
            cancellation = "источник не вернул подтверждённый тариф"
        detail_rows = [
            ("ID", item.get("hotelId") or "источник не вернул"),
            (
                "Ссылка T-Bank",
                f"[Открыть карточку отеля]({tbank_url})"
                if tbank_url else "источник не вернул",
            ),
            ("Звёзды", stars if stars else "источник не вернул"),
            ("Рейтинг", rating if rating is not None else "источник не вернул"),
            (
                "Количество отзывов",
                review_count if review_count is not None else "источник не вернул",
            ),
            ("Адрес", _cut(details.get("address") or "", 180) or "источник не вернул"),
            ("Описание", _cut(details.get("description") or "", 420) or "источник не вернул"),
            (
                "Заезд / выезд",
            ...
            (
                "Удобства",
                "; ".join(details.get("facilities") or []) or "источник не вернул",
            ),
            ("Ночей", item.get("nights") or "источник не вернул"),
            ("Полная цена", price_text),
            ("Цена за ночь", nightly_text),
            (
                "Номер",
                (rate.get("room") or "источник не вернул") if rate.get("available") else
                "источник не вернул подтверждённый тариф",
            ),
            (
                "Кровать",
                (rate.get("bed") or "источник не вернул") if rate.get("available") else
                "источник не вернул подтверждённый тариф",
            ),
            (
                "Питание",
                (rate.get("meal") or "источник не вернул") if rate.get("available") else
                "источник не вернул подтверждённый тариф",
            ),
            (
                "Оплата",
                (rate.get("payment") or "источник не вернул") if rate.get("available") else
                "источник не вернул подтверждённый тариф",
            ),
            ("Отмена", cancellation),
        ]
        lines.extend([
            f"\n## {position}. {_hotel_markdown_cell(name)}",
            "",
            "### Детали отеля",
            "",
            "| Параметр | Значение |",
        ...
        for label, value in detail_rows:
            rendered_value = (
                str(value) if label == "Ссылка T-Bank" and tbank_url
                else _hotel_markdown_cell(value)
            )
            lines.append(f"| {_hotel_markdown_cell(label)} | {rendered_value} |")
        lines.extend([
            "",
            "### Фотографии",
            "",
        ...
        for index in range(3):
            if index < len(chat_photos):
                ...
                photo_cells.append(
                    f"![{_hotel_photo_alt(name)} — "
                    f"{_hotel_photo_alt(caption).lower()} {index + 1}]"
                    f"({photo.get('url')})<br>{_hotel_markdown_cell(caption)}"
                )
            else:
                photo_cells.append(
                    "Фото недоступно<br>Источник не вернул фотографию "
                    f"{index + 1}.")
        lines.append("| " + " | ".join(photo_cells) + " |")
        digest = item.get("reviewDigest") or _hotel_review_digest([])
        period = ""
        if digest.get("dateFrom") or digest.get("dateTo"):
            period = f", {digest.get('dateFrom') or '?'}—{digest.get('dateTo') or '?'}"
        sample_text = f"{digest.get('sampleSize', 0)} отзывов{period}"
        lines.extend([
            "",
            "### Овервью отзывов",
            "",
            "| Параметр | Значение |",
            "|---|---|",
            f"| Выборка | {_hotel_markdown_cell(sample_text)} |",
            f"| Резюме | {_hotel_markdown_cell(digest.get('summary') or 'недостаточно данных')} |",
            "| Плюсы | " + _hotel_markdown_cell("; ".join(
                digest.get("pluses") or ["недостаточно данных"])) + " |",
            "| Минусы | " + _hotel_markdown_cell("; ".join(
                digest.get("minuses") or ["недостаточно данных"])) + " |",
            "| Кому подходит | " + _hotel_markdown_cell("; ".join(
                digest.get("suitableFor") or ["недостаточно данных"])) + " |",
        ])
...
def hotel_search(destination_id: int, checkin_date: str, checkout_date: str,
    ...
    реальными фото, тарифами и reviewDigest. comparisonMarkdown задаёт строгий
    hotel-chat/v2: краткое сравнение, таблицу деталей, трёхколоночную
    фотогалерею и таблицу обзора отзывов для каждой chat-карточки.
    ...
    try:
        ...
        if not 1 <= int(comparison_limit) <= 5:
            raise TbankApiError(
                "BAD_COMPARISON_LIMIT", "comparison_limit должен быть от 1 до 5.")
        start = _hotel_date(checkin_date, "checkin_date")
        end = _hotel_date(checkout_date, "checkout_date")
        if end <= start:
        ...
        for row in shown:
            ...
            rows.append(item)
        enriched_shortlist, enrichment_warnings = _hotel_search_enriched_shortlist(
            s, rows[:int(comparison_limit)], checkin_date, checkout_date,
            adults, ages, nights,
        )
        ...
        if fmt == "json":
            ...
            payload = _json_envelope({
                "chatFormatVersion": "hotel-chat/v2",
...
def hotel_details(hotel_id: str, max_facilities: int = 40, max_images: int = 3,
    ...
    try:
        ...
        stars = int(hotel.get("starRating") or 0)
        out = [f"{_flat(hotel.get('hotelName') or '?')} {'★' * stars} | hotel_id={hotel_id}"]
        address = _hotel_address(hotel)
        if address:
            out.append(f"Адрес: {address}")
        ...
        if facilities:
            shown = facilities[:max_facilities] if max_facilities > 0 else facilities
            out.append("Удобства: " + "; ".join(shown))
    ...
    try:
        ...
        if fmt == "json":
            return _json_envelope(
                ...
                meta={"complete": result["complete"]},
            )
        data = result["data"]
        anchor = data["anchor"]
        lines = [
            f"Рестораны рядом: {anchor['name']} · Яндекс Карты",
        ]
        for place in data["restaurants"]:
            details = []
            if place.get("rating") is not None:
                rating = f"★ {place['rating']:g}/5"
                if place.get("reviewCount") is not None:
                    rating += f" · {place['reviewCount']} отзывов"
                details.append(rating)
            if place.get("priceLevel"):
                details.append(place["priceLevel"])
            if place.get("distanceMeters") is not None:
                details.append(f"{place['distanceMeters']} м")
            lines.append(f"- {place['name']} | {' | '.join(details)}")
            if place.get("address"):
                lines.append(f"  {place['address']}")
            lines.append(f"  {place['sourceUrl']}")
        lines.extend(f"⚠️ {warning}" for warning in result["warnings"])
        return "\n".join(lines)
    except Exception as exc:
        return _formatted_error(exc, response_format, source="Yandex Maps")


@_threaded_tool
def nearby_search(city: str, anchor_name: str = "", address: str = "",
                  latitude: float | None = None,
                  longitude: float | None = None,
                  include_poi: bool = False,
                  place_kinds: str = "culture", limit: int = 20,
                  response_format: str = "text") -> str:
    """Рестораны и, опционально, интересные места в радиусе 1,8 км.

    Передай city и либо latitude+longitude, либо address/anchor_name. Без
    координат точка определяется через Nominatim. include_poi=true добавляет
    категории из place_kinds: culture, nature, nightlife (через запятую).
    Данные берутся из OpenStreetMap через Overpass; API-ключ не нужен.
    """
    try:
        fmt = _response_format(response_format)
        result = _search_nearby(
            city=city, anchor_name=anchor_name, address=address,
            latitude=latitude, longitude=longitude,
            include_poi=include_poi, place_kinds=place_kinds, limit=limit)
        if fmt == "json":
            return _json_envelope(
        ...
        anchor = data["anchor"]
        lines = [
            (f"Места рядом: {anchor['name']} · {data['radiusMeters']} м · "
             f"OpenStreetMap"),
        ]
        for place in data["places"]:
            details = [
                f"{place['distanceMeters']} м",
                place["category"],
            ]
            if place["cuisine"]:
                details.append(f"кухня: {place['cuisine']}")
            if place["address"]:
                details.append(place["address"])
            if place["openingHours"]:
                details.append(f"часы: {place['openingHours']}")
            details.append(place["sourceUrl"])
            lines.append(f"- {place['name']} | " + " | ".join(details))
        if not data["places"]:
            lines.append("В радиусе 1,8 км подходящих мест в OpenStreetMap не найдено.")
        lines.extend(f"⚠️ {warning}" for warning in result["warnings"])
        return "\n".join(lines)
    except Exception as e:
        return _formatted_error(e, response_format, source="OpenStreetMap")
...
def weather(city: str, latitude: float, longitude: float,
    ...
    Для ближайших 16 дней возвращается forecast Open-Meteo; для остальных —
    climate по архиву ERA5 за 1991–2020. Вид данных явно указан для каждого дня.
    Если один источник недоступен, возвращается доступная часть с предупреждением.
    """
    try:
        fmt = _response_format(response_format)
        result = _weather_report(
            city=city, latitude=latitude, longitude=longitude,
            date_from=date_from, date_to=date_to)
        if not result["data"]["days"]:
            raise TbankApiError(
                "SOURCE_UNAVAILABLE",
                "Open-Meteo не вернул ни прогноз, ни климатическую оценку.")
        if fmt == "json":
            return _json_envelope(
                result["data"], source=result["source"],
                warnings=result["warnings"],
        ...
        lines = [
            (f"Погода: {result['data']['city']} · {date_from}—{date_to} · "
             f"{result['source']}"),
        ]
        for day in result["data"]["days"]:
            if day["kind"] == "forecast":
                lines.append(
                    f"- {day['date']} · forecast · "
                    f"{day['temperatureMinC']:+}…{day['temperatureMaxC']:+} °C · "
                    f"осадки {day['precipitationProbabilityPct']}% · "
                    f"WMO {day['weatherCode']}")
            else:
                lines.append(
                    f"- {day['date']} · climate · "
                    f"{day['temperatureMinC']:+}…{day['temperatureMaxC']:+} °C · "
                    f"частота осадков {day['precipitationFrequencyPct']}% · "
                    f"наблюдений {day['sampleSize']}")
        lines.extend(f"⚠️ {warning}" for warning in result["warnings"])
        return "\n".join(lines)
    except Exception as e:
        return _formatted_error(e, response_format, source="Open-Meteo")
...
def train_stations(search_text: str, limit: int = 20,
    ...
    searchCode можно напрямую передать в origin/destination инструмента
    train_search. Предпочитай результат type=City, если нужен весь город.
    """
    try:
        fmt = _response_format(response_format)
        result = _station_suggestions(search_text=search_text, limit=limit)
        if fmt == "json":
            return _json_envelope(
                result["data"], source=result["source"],
                warnings=result["warnings"],
                meta={"complete": result["complete"]})
        rows = result["data"]["stations"]
        lines = [f"ЖД-станции: {search_text}"]
        lines.extend(
            f"- {row['station']} | {row['city']}, {row['region']} | "
            f"type={row['type']} | searchCode={row['searchCode']}"
            for row in rows)
        if not rows:
            lines.append("Станций не найдено.")
        lines.extend(f"⚠️ {warning}" for warning in result["warnings"])
        return "\n".join(lines)
    except Exception as e:
        return _formatted_error(e, response_format, source="T-Bank Railways")
...
def train_search(origin: str, destination: str, date: str, adults: int = 1,
    ...
    2004000 — Санкт-Петербург), date — YYYY-MM-DD. Если код неизвестен, сначала
    вызови train_stations и возьми его поле searchCode.

    Возвращает расписание, минимальную цену и число доступных мест. MCP ничего
    не бронирует и не оплачивает; банковская сессия для поиска не нужна.
    """
    try:
        fmt = _response_format(response_format)
        ...
        if len(shown) < len(trains):
            warnings.append(f"Показано {len(shown)} из {len(trains)} поездов.")
        if fmt == "json":
            data = {key: value for key, value in result["data"].items() if key != "ways"}
            data.update({
                "trains": [{key: value for key, value in train.items()
                            if key != "priceDecimal"} for train in shown],
                "total": len(trains),
            })
            return _json_envelope(
                data, source=result["source"], warnings=warnings,
                meta={"complete": len(shown) == len(trains)})

        def render(train):
            dep = train["departureAt"][11:16]
            arr = train["arrivalAt"][11:16]
            money = (f"от {train['minPrice']:.0f} ₽"
                     + (f" (мест {train['availableSeats']})" if train["availableSeats"] else "")
                     if train["minPrice"] is not None else "мест нет")
            return (f"- {dep}→{arr} | № {train['trainNumber'] or '?'} "
                    f"{_cut(train['name'], 20)} | "
                    f"{_cut(train['origin']['name'], 22)} → "
                    f"{_cut(train['destination']['name'], 22)} | {money}")

        return _rows_out(trains, render, limit=limit, total=len(trains),
                         header=f"Поезда {origin}→{destination} на {date}",
                         order_note="как отдал банк",
                         more_hint=f"Передай limit={len(trains)}.")
    except Exception as e:
        return _formatted_error(e, response_format, source="T-Bank Railways")
...
@mcp.tool()
def flight_history(response_format: str = "text") -> str:
    ...
    try:
        ...
        def place_data(place):
            ...
            return {
                "name": ((city_name.get("ru") if isinstance(city_name, dict) else "")
                         or (name.get("ru") if isinstance(name, dict) else "") or ""),
                "code": str(place.get("code") or place.get("city_code") or ""),
            }
        if fmt == "json":
            return _json_envelope({
                "searches": [{
                    "searchedAt": str(row.get("searchTime") or ""),
                    "from": place_data(row.get("from")),
                    "to": place_data(row.get("to")),
...
async def compare_flight_prices(
    ...
    sort_by: Literal["price", "duration", "departure_time"] = "price",
    limit: int = 10,
    ...
    """
    requested = len(dates) if isinstance(dates, list) else 0
    baseline_key = str(dates[0]) if requested else ""
    try:
        dates, initial_warnings = _comparison_dates(dates, maximum=7)
        baseline_key = dates[0]
        limit = _comparison_limit(limit)
        _flight_passengers(adults, children, infants)
        # Validate filters before starting any live request.
        _eligible_flights(
        ...
        async def load(departure_date: str):
            try:
                return await asyncio.to_thread(
                    _flight_inventory_call, session, from_code, to_code,
                    departure_date, adults, children, infants, only_bookable)
            except Exception as exc:  # one date must not cancel all other dates
                return exc

        results = await asyncio.gather(*(load(value) for value in dates))
        failures: list[ComparisonFailure] = []
        observed: list[ObservedGroup] = []
        all_rows: list[dict] = []
        warnings = list(initial_warnings)
        sources, checked = set(), []
        succeeded = incomplete = 0
        for departure_date, result in zip(dates, results):
            if isinstance(result, BaseException):
                failures.append(_failure(departure_date, result))
                observed.append(ObservedGroup(
                    key=departure_date, label=departure_date, status="failed"))
                continue
            succeeded += 1
            incomplete += int(not result["complete"])
            sources.add(result["source"]); checked.append(result["checkedAt"])
            warnings.extend(result["warnings"])
            rows, filter_warnings = _eligible_flights(
                result["rows"], max_stops=max_stops,
            ...
            observed.append(ObservedGroup(
                ...
                status=_comparison_status(rows, result["complete"]),
                prices=[row["priceDecimal"] for row in rows],
            ))

        groups, baseline, lowest = comparison_groups(observed)
        ranked = sort_flights(all_rows, sort_by)
        shown = ranked[:limit]
        items: list[FlightComparisonItem] = []
        for rank, row in enumerate(shown, 1):
            delta_rub, delta_pct = price_delta(row["priceDecimal"], lowest)
            items.append(FlightComparisonItem(
                rank=rank, group_key=row["groupKey"], date=row["date"],
                offer_id=row["offerId"], price_rub=row["price"],
                delta_rub_from_lowest_observed=delta_rub or 0.0,
                delta_pct_from_lowest_observed=delta_pct or 0.0,
                summary=row["summary"], departure_at=row["departureAt"],
                arrival_at=row["arrivalAt"],
                duration_minutes=int(row["durationMinutes"]), stops=int(row["stops"]),
                with_baggage=bool(row["withBaggage"]),
                refundable=bool(row["refundable"]), vendor=row["vendor"],
                source=row["source"], checked_at=row["checkedAt"],
            ))
        complete = not failures and incomplete == 0
        if baseline <= 0 and all_rows:
            warnings.append(
                "В первой запрошенной дате нет сравнимой цены; deltaFromBaseline не рассчитана.")
        warnings = _comparison_warnings(
            warnings, complete=complete, shown=len(shown), total=len(ranked))
        data = FlightComparisonData(
            from_code=from_code.upper(), to_code=to_code.upper(), dates=dates,
            sort_by=sort_by, groups=groups, items=items, failures=failures)
        ok = bool(all_rows)
        return FlightComparisonResponse(
            ok=ok, data=data,
            source=" + ".join(sorted(sources)) or "T-Bank Avia",
            checked_at=max(checked, default=_checked_at()), warnings=warnings,
            meta=_comparison_meta(
                complete=complete, requested=len(dates), succeeded=succeeded,
                failed=len(failures), incomplete=incomplete, baseline_key=baseline_key),
            error=(None if ok else ComparisonError(
                code="NO_COMPARABLE_RESULTS",
                message="Ни для одной даты не получено сравнимых авиапредложений.")),
        )
    except Exception as exc:
        failure = _failure("compare_flight_prices", exc)
        return FlightComparisonResponse(
            ok=False, data=None, source="T-Bank Avia", checked_at=_checked_at(),
            warnings=[], meta=_comparison_meta(
                complete=False, requested=requested, succeeded=0, failed=requested,
                incomplete=0, baseline_key=baseline_key),
            error=ComparisonError(code=failure.code, message=failure.message),
        )


@mcp.tool()
async def compare_train_prices(
    origin: str,
    destination: str,
    dates: list[str],
    adults: int = 1,
    ...
    """Сравнить поезда для 1–5 дат по тарифу, длительности или отправлению.

    origin/destination — searchCode из train_stations. Цена — минимальный
    наблюдаемый тариф на место, а не обещание итоговой цены для всей группы.
    """
    requested = len(dates) if isinstance(dates, list) else 0
    baseline_key = str(dates[0]) if requested else ""
    try:
        dates, initial_warnings = _comparison_dates(dates, maximum=5)
        baseline_key = dates[0]
        limit = _comparison_limit(limit)
        _flight_passengers(adults, children, 0)
        if max_duration_minutes is not None and int(max_duration_minutes) <= 0:
            raise TbankApiError(
                "BAD_FILTER", "max_duration_minutes должен быть положительным.")

        async def load(departure_date: str):
            try:
                return await asyncio.to_thread(
                    _train_inventory_call, origin, destination, departure_date,
                    adults, children)
            except Exception as exc:
                return exc
        ...
        for departure_date, result in zip(dates, results):
            ...
            rows = [row for row in result["rows"]
                    if row["priceDecimal"] > 0 and
                    (max_duration_minutes is None or
                     int(row.get("durationMinutes") or 0) <= int(max_duration_minutes))]
            for row in rows:
                row.update({
                    "groupKey": departure_date, "date": departure_date,
                    "source": result["source"], "checkedAt": result["checkedAt"],
                })
            all_rows.extend(rows)
            observed.append(ObservedGroup(
                key=departure_date, label=departure_date,
                status=_comparison_status(rows, result["complete"]),
                prices=[row["priceDecimal"] for row in rows]))

        groups, baseline, lowest = comparison_groups(observed)
        ranked = sort_trains(all_rows, sort_by)
        shown = ranked[:limit]
        items: list[TrainComparisonItem] = []
        for rank, row in enumerate(shown, 1):
            delta_rub, delta_pct = price_delta(row["priceDecimal"], lowest)
            items.append(TrainComparisonItem(
                rank=rank, group_key=row["groupKey"], date=row["date"],
                train_number=row["trainNumber"], name=row["name"],
                origin_name=row["origin"]["name"],
                destination_name=row["destination"]["name"],
                departure_at=row["departureAt"], arrival_at=row["arrivalAt"],
                duration_minutes=int(row["durationMinutes"]),
                min_fare_rub=float(row["minPrice"]),
                available_seats=int(row["availableSeats"]),
                car_types=list(row["carTypes"]),
                delta_rub_from_lowest_observed=delta_rub or 0.0,
                delta_pct_from_lowest_observed=delta_pct or 0.0,
                source=row["source"], checked_at=row["checkedAt"],
            ))
        ...
        if baseline <= 0 and all_rows:
            warnings.append(
                "В первой запрошенной дате нет сравнимого тарифа; deltaFromBaseline не рассчитана.")
        warnings = _comparison_warnings(
            warnings, complete=complete, shown=len(shown), total=len(ranked))
        data = TrainComparisonData(
            origin=origin, destination=destination, dates=dates, sort_by=sort_by,
            groups=groups, items=items, failures=failures)
        ok = bool(all_rows)
        return TrainComparisonResponse(
            ok=ok, data=data,
            source=" + ".join(sorted(sources)) or "T-Bank Railways",
            checked_at=max(checked, default=_checked_at()), warnings=warnings,
            meta=_comparison_meta(
                complete=complete, requested=len(dates), succeeded=succeeded,
                failed=len(failures), incomplete=incomplete, baseline_key=baseline_key),
            error=(None if ok else ComparisonError(
                code="NO_COMPARABLE_RESULTS",
                message="Ни для одной даты не получено сравнимых поездов.")),
        )
    except Exception as exc:
        failure = _failure("compare_train_prices", exc)
        return TrainComparisonResponse(
            ok=False, data=None, source="T-Bank Railways", checked_at=_checked_at(),
            warnings=[], meta=_comparison_meta(
                complete=False, requested=requested, succeeded=0, failed=requested,
                incomplete=0, baseline_key=baseline_key),
            error=ComparisonError(code=failure.code, message=failure.message),
...
async def compare_hotel_prices(
    ...
    min_stars: int = 0,
    min_rating: float = 0,
    max_total_price_rub: float | None = None,
    sort_by: Literal["total_price", "nightly_price", "rating"] = "total_price",
    limit: int = 10,
    ...
    baseline_key = ""
    try:
        windows, initial_warnings = _comparison_windows(windows, maximum=7)
        baseline_key = windows[0].key
        limit = _comparison_limit(limit)
        if int(destination_id) <= 0:
            raise TbankApiError("BAD_DESTINATION", "destination_id должен быть положительным.")
        ages = _hotel_guests(adults, children_ages)
        wanted_ids = {str(value or "").strip() for value in (hotel_ids or [])}
        if "" in wanted_ids or any(not value.isdigit() for value in wanted_ids):
            raise TbankApiError("BAD_HOTEL_ID", "hotel_ids должны состоять из цифр.")
        _eligible_hotels(
        ...
        async def load(window: StayWindow):
            try:
                return await asyncio.to_thread(
                    _hotel_inventory_call, session, int(destination_id), window,
                    adults, ages)
            except Exception as exc:
                return exc

        results = await asyncio.gather(*(load(window) for window in windows))
        failures: list[ComparisonFailure] = []
        observed: list[ObservedGroup] = []
        all_rows: list[dict] = []
        warnings = list(initial_warnings)
        ...
        for window, result in zip(windows, results):
            if isinstance(result, BaseException):
                failures.append(_failure(window.key, result))
                observed.append(ObservedGroup(
                    key=window.key, label=window.key, status="failed"))
                continue
            succeeded += 1
            incomplete += int(not result["complete"])
            sources.add(result["source"]); checked.append(result["checkedAt"])
            warnings.extend(result["warnings"])
            rows, filter_warnings = _eligible_hotels(
                result["rows"], nights=window.nights, hotel_ids=wanted_ids,
                min_stars=min_stars, min_rating=min_rating,
                max_total_price_rub=max_total_price_rub)
            warnings.extend(filter_warnings)
            for row in rows:
                row.update({
                    "groupKey": window.key,
                    "checkinDate": window.checkin_date,
                    "checkoutDate": window.checkout_date,
                    "nights": window.nights,
                    "source": result["source"],
                    "checkedAt": result["checkedAt"],
                })
            all_rows.extend(rows)
            observed.append(ObservedGroup(
                key=window.key, label=window.key,
                status=_comparison_status(rows, result["complete"]),
                prices=[row["priceDecimal"] for row in rows]))

        groups, baseline, lowest = comparison_groups(observed)
        ranked = sort_hotels(all_rows, sort_by)
        shown = ranked[:limit]
        items: list[HotelComparisonItem] = []
        for rank, row in enumerate(shown, 1):
            delta_rub, delta_pct = price_delta(row["priceDecimal"], lowest)
            items.append(HotelComparisonItem(
                rank=rank, group_key=row["groupKey"],
                checkin_date=row["checkinDate"], checkout_date=row["checkoutDate"],
                nights=int(row["nights"]), hotel_id=row["hotelId"], name=row["name"],
                stars=int(row["stars"]), rating=row.get("rating"),
                address=row["address"], meal=row["meal"],
        ...
        if baseline <= 0 and all_rows:
            warnings.append(
                "В первом окне нет сравнимой цены; deltaFromBaseline не рассчитана.")
        warnings = _comparison_warnings(
            warnings, complete=complete, shown=len(shown), total=len(ranked))
        data = HotelComparisonData(
            destination_id=int(destination_id),
            windows=[_window_value(window) for window in windows], sort_by=sort_by,
            groups=groups, items=items, failures=failures)
        ok = bool(all_rows)
        return HotelComparisonResponse(
            ok=ok, data=data,
            source=" + ".join(sorted(sources)) or "T-Bank Hotels",
            checked_at=max(checked, default=_checked_at()), warnings=warnings,
            meta=_comparison_meta(
                complete=complete, requested=len(windows), succeeded=succeeded,
                failed=len(failures), incomplete=incomplete, baseline_key=baseline_key),
            error=(None if ok else ComparisonError(
                code="NO_COMPARABLE_RESULTS",
                message="Ни для одного окна не получено сравнимых отелей.")),
        )
    except Exception as exc:
        failure = _failure("compare_hotel_prices", exc)
        return HotelComparisonResponse(
            ok=False, data=None, source="T-Bank Hotels", checked_at=_checked_at(),
            warnings=[], meta=_comparison_meta(
                complete=False, requested=requested, succeeded=0, failed=requested,
                incomplete=0, baseline_key=baseline_key),
            error=ComparisonError(code=failure.code, message=failure.message),
...
@mcp.tool()
async def compare_flight_hotel_prices(
    from_code: str,
    to_code: str,
    ...
    baggage_required: bool = False,
    refundable_required: bool = False,
    min_hotel_stars: int = 0,
    min_hotel_rating: float = 0,
    max_hotel_price_rub: float | None = None,
    ...
    baseline_key = ""
    try:
        windows, initial_warnings = _comparison_windows(windows, maximum=3)
        baseline_key = windows[0].key
        limit = _comparison_limit(limit, maximum=10)
        if int(hotel_destination_id) <= 0:
            raise TbankApiError(
                "BAD_DESTINATION", "hotel_destination_id должен быть положительным.")
        ages = _hotel_guests(adults, children_ages)
        flight_children = sum(age >= 2 for age in ages)
        flight_infants = sum(age < 2 for age in ages)
        _flight_passengers(adults, flight_children, flight_infants)
        _eligible_flights(
            [], max_stops=max_stops, max_duration_minutes=None,
            baggage_required=baggage_required, refundable_required=refundable_required)
        _eligible_hotels(
            [], nights=1, hotel_ids=set(), min_stars=min_hotel_stars,
            min_rating=min_hotel_rating, max_total_price_rub=max_hotel_price_rub)
        ...
        async def safe(scope: str, fn, *args):
            try:
                return scope, await asyncio.to_thread(fn, *args)
            except Exception as exc:
                return scope, exc
        ...
        for window in windows:
            tasks.extend([
                safe(f"{window.key}:outbound", _flight_inventory_call,
                     session, from_code, to_code, window.checkin_date,
                     adults, flight_children, flight_infants, only_bookable),
                safe(f"{window.key}:return", _flight_inventory_call,
                     session, to_code, from_code, window.checkout_date,
                     adults, flight_children, flight_infants, only_bookable),
                safe(f"{window.key}:hotel", _hotel_inventory_call,
                     session, int(hotel_destination_id), window, adults, ages),
            ])
        loaded = dict(await asyncio.gather(*tasks))
        failures: list[ComparisonFailure] = []
        observed: list[ObservedGroup] = []
        bundles: list[dict] = []
        warnings = list(initial_warnings)
        ...
        succeeded = incomplete = 0
        for window in windows:
            keys = [f"{window.key}:outbound", f"{window.key}:return", f"{window.key}:hotel"]
            values = [loaded[key] for key in keys]
            failed_here = False
            for scope, value in zip(keys, values):
                if isinstance(value, BaseException):
                    failures.append(_failure(scope, value)); failed_here = True
                else:
                    succeeded += 1
                    incomplete += int(not value["complete"])
                    sources.add(value["source"]); checked.append(value["checkedAt"])
                    warnings.extend(value["warnings"])
            if failed_here:
                observed.append(ObservedGroup(
                    key=window.key, label=window.key, status="failed"))
                continue
            outbound_result, return_result, hotel_result = values
            outbound, outbound_warnings = _eligible_flights(
                outbound_result["rows"], max_stops=max_stops,
                max_duration_minutes=None, baggage_required=baggage_required,
                refundable_required=refundable_required)
            returning, return_warnings = _eligible_flights(
                return_result["rows"], max_stops=max_stops,
                max_duration_minutes=None, baggage_required=baggage_required,
                refundable_required=refundable_required)
            hotels, hotel_warnings = _eligible_hotels(
                hotel_result["rows"], nights=window.nights, hotel_ids=set(),
                min_stars=min_hotel_stars, min_rating=min_hotel_rating,
                max_total_price_rub=max_hotel_price_rub)
            warnings.extend(outbound_warnings + return_warnings + hotel_warnings)
            source_complete = all(value["complete"] for value in values)
            if not outbound or not returning or not hotels:
                observed.append(ObservedGroup(
                    key=window.key, label=window.key,
                    status="partial" if not source_complete else "empty"))
                continue
            best_outbound = sort_flights(outbound, "price")[0]
            best_return = sort_flights(returning, "price")[0]
            group_bundles = []
            for hotel in hotels:
                total = (best_outbound["priceDecimal"] + best_return["priceDecimal"]
                         + hotel["priceDecimal"])
                group_bundles.append({
                    ...
                    "totalDecimal": total,
                    "source": " + ".join(sorted({
                        outbound_result["source"], return_result["source"],
                        hotel_result["source"],
                    })),
                    "checkedAt": max(outbound_result["checkedAt"],
                                     return_result["checkedAt"], hotel_result["checkedAt"]),
                })
            bundles.extend(group_bundles)
            observed.append(ObservedGroup(
                key=window.key, label=window.key,
                status="complete" if source_complete else "partial",
                prices=[bundle["totalDecimal"] for bundle in group_bundles]))

        groups, baseline, lowest = comparison_groups(observed)
        ranked = sorted(
            bundles,
            key=lambda row: (row["totalDecimal"], row["groupKey"],
                             row["hotel"].get("hotelId") or ""),
        )
        shown = ranked[:limit]
        items: list[FlightHotelComparisonItem] = []
        for rank, row in enumerate(shown, 1):
            delta_rub, delta_pct = price_delta(row["totalDecimal"], lowest)
            hotel, outbound, returning = row["hotel"], row["outbound"], row["return"]
            items.append(FlightHotelComparisonItem(
                rank=rank, group_key=row["groupKey"],
                checkin_date=row["window"].checkin_date,
                checkout_date=row["window"].checkout_date,
                nights=row["window"].nights,
                outbound_offer_id=outbound["offerId"],
                outbound_summary=outbound["summary"],
                return_offer_id=returning["offerId"],
                return_summary=returning["summary"],
                hotel_id=hotel["hotelId"], hotel_name=hotel["name"],
                hotel_stars=int(hotel["stars"]), hotel_rating=hotel.get("rating"),
                outbound_flight_rub=outbound["price"],
                return_flight_rub=returning["price"], hotel_rub=hotel["price"],
                bundle_total_rub=rub_number(row["totalDecimal"]),
                delta_to_budget_rub=(rub_number(row["totalDecimal"] - budget)
        ...
        data = FlightHotelComparisonData(
            ...
            hotel_destination_id=int(hotel_destination_id),
            windows=[_window_value(window) for window in windows],
            priced_components=["outboundFlight", "returnFlight", "hotel"],
            groups=groups, items=items, failures=failures)
        ok = bool(bundles)
        return FlightHotelComparisonResponse(
            ok=ok, data=data,
            source=" + ".join(sorted(sources)) or "T-Bank Avia + T-Bank Hotels",
            checked_at=max(checked, default=_checked_at()), warnings=warnings,
            meta=_comparison_meta(
                complete=complete, requested=requested_searches, succeeded=succeeded,
                failed=len(failures), incomplete=incomplete, baseline_key=baseline_key),
            error=(None if ok else ComparisonError(
                code="NO_COMPARABLE_RESULTS",
                message="Не получено ни одной полной сборки перелёт + отель.")),
        )
    except Exception as exc:
        failure = _failure("compare_flight_hotel_prices", exc)
        requested_searches = requested_windows * 3
        return FlightHotelComparisonResponse(
            ok=False, data=None, source="T-Bank Avia + T-Bank Hotels",
            checked_at=_checked_at(), warnings=[],
            meta=_comparison_meta(
                complete=False, requested=requested_searches, succeeded=0,
                failed=requested_searches, incomplete=0, baseline_key=baseline_key),
            error=ComparisonError(code=failure.code, message=failure.message),
        )
        ...
        if fmt == "json":
            ...
            for event in shown:
                ...
                row = {
                    ...
                    "ageRestriction": str(fields.get("ageRestriction") or ""),
                    "rating": (_hotel_amount(rating.get("value") if isinstance(rating, dict) else rating)
                               if rating else None),
                    "slots": slots,
                }
                if image_url:
            ...
            warnings = []
            if scanned < amount:
                warnings.append(f"Просмотрено {scanned} из {amount} событий.")
            if len(shown) < len(events):
                warnings.append(f"Показано {len(shown)} из {len(events)} найденных событий.")
            return _json_envelope({
                "kind": kind,
                "city": city,
                "dateFrom": date_from,
...
def afisha_places(kind: str = "movie", city: str = "", query: str = "",
    ...
    try:
        ...
        if fmt == "json":
            ...
            rows = []
            for place in shown:
                latitude, longitude = _hotel_coordinates({"geo": place.get("geo") or place})
                rows.append({
                    "objectId": str(place.get("id") or ""),
                    "name": _flat(place.get("name") or ""),
                    "address": _flat(place.get("address") or ""),
                    "latitude": latitude,
                    "longitude": longitude,
                    "subways": [str(item.get("name") or "") for item in
                                (place.get("subways") or []) if isinstance(item, dict)],
                })
            warnings = ([] if len(shown) == len(places) else
                        [f"Показано {len(shown)} из {len(places)} полученных площадок."])
            return _json_envelope({
                "kind": kind,
                "city": city,
                "total": total,
               ...
               meta={"complete": len(places) >= total})
        if not places:
...
_FLOW_KEYWORDS = {
    ...
    "hotels": "отель отели гостиница гостиницы номер номера заселение проживание",
...
def flows(topic: str = "") -> str:
    ...
    «карты», «заказы», «кбжу», «инвест», «кредит», «чат», «поиск», «отели», «логин».
