...
def server_instructions() -> str:
    ...
    return (
        ...
        "в текущей папке. Если host не показывает resources, вызови "
        "list_instructions(), затем read_instruction(\"tbank-trip-generation\") "
        "и нужный узкий travel-skill. Для hotel-задач hotel_search автоматически "
        "возвращает enrichedShortlist: детали, реальные фото, актуальные тарифы "
        "и выборку отзывов с плоскими полями pluses, minuses, suitableFor для "
        "первых comparison_limit карточек. hotel_search намеренно не публикует "
        "неполные каталоговые карточки: выбирай только из enrichedShortlist. В "
        "видимом ответе для КАЖДОГО показанного отеля обязательно вставь минимум "
        "одно изображение из primaryPhotoUrl/photoUrls и явно выведи «Плюсы», "
        "«Минусы», «Кому подходит»; отсутствие данных обозначь предупреждением. "
        "Не добавляй в финальное сравнение карточку, для которой эти четыре "
        "элемента не показаны. hotel_search намеренно возвращает только URL "
        "источников и не прикладывает готовые image-блоки/data URI, чтобы host "
        "не принял скрытое внутри tool result фото за изображение в ответе. "
        "Все URL изображений, которые "
        "вставляются моделью напрямую в видимый chat/Markdown/HTML, собери и "
        "одним вызовом передай в image_to_data_uri(urls=[...]); используй "
        "успешные dataUri и не вызывай tool последовательно для каждого URL; "
        "канонические правила находятся в travel-output-modes. Для "
        "get_trip_report отдельные вызовы не нужны: report применяет тот же "
        "конвертер внутри. Для HTML или Markdown после этого "
        "вызови get_trip_report. Не выдумывай цены, "
        "идентификаторы, "
