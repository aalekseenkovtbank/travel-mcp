...
def server_instructions() -> str:
    ...
    return (
        ...
        "видимом ответе для КАЖДОГО показанного отеля используй без перестановок "
        "строгий табличный шаблон hotel-chat/v2 из comparisonMarkdown: сначала "
        "краткое сравнение, затем для каждого отеля таблица «Детали отеля», "
        "трёхколоночная таблица «Фотографии» и таблица «Овервью отзывов» с полями «Выборка», "
        "«Резюме», «Плюсы», «Минусы», «Кому подходит». Не сокращай и не переименовывай "
        "секции. Если источник вернул меньше трёх фото или другую деталь, сохрани "
        "слот и покажи предупреждение, ничего не выдумывая. hotel_search также "
        "прикладывает до трёх нативных MCP image-блоков на отель. Для HTML или Markdown после этого "
