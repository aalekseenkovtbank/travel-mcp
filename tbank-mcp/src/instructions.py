...
def server_instructions() -> str:
    ...
    return (
        ...
        "и нужный узкий travel-skill. КРИТИЧЕСКИЙ HOTEL FLOW: hotel_search — "
        "только промежуточный поиск, а не готовый ответ. Любой запрос на подбор "
        "или сравнение отелей заверши только после hotel_latest_offers для "
        "shortlist и hotel_rates плюс hotel_reviews для каждого финального "
        "отеля. Каждый hotel-тул возвращает details с карточкой и фотографиями. "
        "В видимом ответе покажи детали, хотя бы первое доступное реальное фото "
        "и обзор выборки отзывов с отдельными полями «Плюсы», «Минусы», «Кому "
        "подходит»; отсутствие данных обозначь предупреждением. Для HTML или "
        "Markdown после этого вызови get_trip_report. Не выдумывай цены, "
        "идентификаторы, "
