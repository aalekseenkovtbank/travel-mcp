...
def record(tool: str, args: dict, started: float, result, error: str | None) -> None:
    ...
    try:
        ...
        if isinstance(structured, dict):
            ...
            head = (f"<structured ok={bool(structured.get('ok', True))} "
                    f"groups={len(groups)} items={len(items)} warnings={len(warnings)}>")
            if structured.get("ok") is False and not error:
                error = "StructuredToolError"
                detail = structured.get("error")
                if isinstance(detail, dict) and detail.get("code"):
                    _last_error["code"] = str(detail["code"])[:64]
        elif text.strip():
