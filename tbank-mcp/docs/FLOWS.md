...
4. `audience_profile()` → only a coarse age band and whether 18+ content is
   allowed; exact birth date and sex never leave the MCP process.
...
1. `train_stations(search_text)` calls the public T-Bank fulltext search with
   `searchTypes=["railway_stations"]`. Pick its numeric `searchCode`; prefer
   `type=City` when the user named a city rather than a particular station.
2. `train_search(origin, destination, date)` sends those codes to the public
   Railways search API and returns trains, minimum prices and available seats.
...
2. `hotel_search(destination_id, checkin_date, checkout_date, adults,
   ...
   дополнительные варианты. В видимом сравнении без изменений используй строгий
   `comparisonMarkdown`: краткая сравнительная таблица, затем одинаковые таблицы
   деталей, трёхколоночные фотогалереи и таблицы отзывов с полями «Выборка»,
   «Резюме», «Плюсы», «Минусы», «Кому подходит» для каждого включённого отеля.
   Версия формата возвращается в `chatFormatVersion` (`hotel-chat/v2`).
   Недостающие фото помечаются в своих слотах. Кроме
   URL, `hotel_search` прикладывает до трёх bounded нативных MCP image-блоков на
   обогащённый отель.
