...
ниже. Не добавляй события, если их не вернул источник. Если `events` пуст,
`get_trip_report()` сам выполняет ограниченный поиск концертов и спектаклей,
подтверждает выбранные показы через расписание и добавляет их в отчёт.
...
4. Для выбранных концертов и спектаклей вызови
   ...
   Оба инструмента расписаний не требуют банковской авторизации. Если расписание
   всё же недоступно из-за ошибки источника, не делай fallback на `search_app`:
   сохрани кандидата в текстовой выдаче, но не включай его в `request.events`
   без подтверждённого `startsAt`.
...
встроенным renderer. Цены и тарифы отелей он не перепроверяет. Для пустого
`events` автоматически загружает ограниченную подборку концертов и спектаклей
Афиши с подтверждёнными сеансами; для пустого `venues` загружает рестораны
Яндекс.Карт. Уже переданные карточки заведений без фото report сопоставляет с
публичными карточками Яндекс.Карт и дополняет фотографиями. Не передавай
`bookHash` в request.

В режиме HTML report сам применяет `image_to_data_uri` pipeline к изображениям
отелей, событий Афиши и ресторанов и встраивает их как `data:` URL. Перед
`get_trip_report` не заменяй HTTPS URL в request: они требуются схемой и остаются
в `documentJson`. Для изображений, которые модель выводит напрямую в chat,
Markdown или самостоятельно собранный HTML, собери уникальные URL и вызови
публичный `image_to_data_uri(urls=[...])` один раз для всего пакета по правилам раздела
[«Фото»](TRAVEL_OUTPUT_MODES.md#фото). При ошибке покажи warning; данные и
изображения не выдумывай.

Для `trip-page/v2` итог всегда содержит ровно три сценария: `balanced`,
`culture` и `food_nightlife`. Переданные вызывающей моделью сценарии этих типов
сохраняются, а отсутствующие `get_trip_report` собирает из фактического
`transport`, выбранного отеля, `events` и `venues`. Автосценарий начинается с
прилёта/прибытия и заселения, добавляет прогулки вокруг уже подтверждённых точек,
мероприятия в их фактическое время и заведения из отчёта, затем заканчивается
выселением и обратным вылетом/отправлением. Для времени заезда и выезда
используются `checkInTime`/`checkOutTime`, когда источник их вернул; иначе текст
