...
В режиме HTML report загружает изображения отелей, событий Афиши и ресторанов
с ограниченного списка CDN и встраивает их как `data:` URL. Исходные HTTPS URL
остаются в `documentJson`; при ошибке загрузки источник сохраняется и добавляется
warning, а данные и изображения не выдумываются.
