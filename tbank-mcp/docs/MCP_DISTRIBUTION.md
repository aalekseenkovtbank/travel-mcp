...
Для контейнерного запуска единого MCP используется защищённая HTTP-точка входа:

```bash
MCP_AUTH_TOKEN='<случайный секрет>' python -m src.http_server
```

Она публикует Streamable HTTP на `/mcp`, оставляет `/health` без авторизации для
проверки платформой и требует `Authorization: Bearer <token>` для MCP-запросов.
TLS завершается на ingress облачной платформы.
