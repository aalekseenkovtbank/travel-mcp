...
Для контейнерного запуска единого MCP используется HTTP-точка входа:

```bash
python -m src.http_server
```

Она публикует Streamable HTTP на `/mcp` и `/health` для проверки платформой.
Авторизация на этой точке входа не выполняется; TLS завершается на ingress
облачной платформы.
