...
`afisha_catalog` публично читает каталоги кино, концертов и театра, а
`cinema_schedule` — сеансы кино без обязательной банковской сессии. Эти запросы
сохраняют обязательный app/device-контекст, но не отправляют Bearer, cookie или
`sessionid`.
