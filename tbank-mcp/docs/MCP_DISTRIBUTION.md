...
В репозитории остаются два entrypoint:

- `src.server` / `tbank-mcp` — полный совместимый сервер с исходной банковской и
  travel-функциональностью;
- `src.travel_mcp.server` / `travel-mcp` — HTTP/stdio launcher того же
  зарегистрированного сервера, используемый для контейнерного деплоя.

## Актуальная HTTP-поверхность

Текущий entrypoint `src.travel_mcp.server` импортирует единый `src.server`.
Устаревший пакет `src.travel_mcp.tools` в запуске не участвует и не требуется
для сборки дистрибутива. Фактический контракт определяют регистрации в
`src.server` и ответ `tools/list`.

## Историческая travel-only allowlist

Ранее планировалась отдельная модульная поверхность из 29 read-only
инструментов:
...
Этот список сохранён только как историческое описание. Он не является текущим
контрактом HTTP launcher.

В этой исторической конфигурации все инструменты были read-only. Поисковые
методы, сравнение, персонализация и рендер не бронируют и не оплачивают;
`hotel_checkout_url()` и `flight_checkout_url()` только возвращают hand-off URL,
который пользователь открывает и проверяет самостоятельно.
...
При инициализации сервер возвращает короткое `instructions` и ресурс
`travel-nova://instructions/index`. Через `resources/list`/`resources/read` или
доступные fallback-инструменты можно читать поставляемые инструкции.
