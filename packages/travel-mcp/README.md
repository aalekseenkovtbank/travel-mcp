...
Сервер публикует 40 инструментов: 39 read-only и локальный renderer. `hotel_search_filters` возвращает
...
`nearby_search` получает рестораны, бары и прогулочные точки из OpenStreetMap
через Nominatim/Overpass; ключи карт для него не нужны.
